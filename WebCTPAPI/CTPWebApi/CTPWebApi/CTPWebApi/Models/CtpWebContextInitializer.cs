...
namespace CTPWebApi.Models
{
    ...
    public class CtpWebContextInitializer : DropCreateDatabaseAlways<CtpWebContext>
    {
        protected override void Seed(CtpWebContext context)
        {
            ...
            Guid WebCategoryId = context.Category.Where(c => c.CategoryName == "Web Components").Select(c => c.CategoryId).SingleOrDefault();
            Guid UtilCategoryId = context.Category.Where(c => c.CategoryName == "Utilities").Select(c => c.CategoryId).SingleOrDefault();
            

            var topics = new List<Topic>()
            {
                new Topic() {TopicName = "GitHub", TopicContext = GitHubContext, DateAdded = DateTime.Now, CategoryId = UtilCategoryId, Order = 1, TopicId = Guid.NewGuid()},
                new Topic() {TopicName = "WebAPI", TopicContext = WebAPIContext, DateAdded = DateTime.Now, CategoryId = WebCategoryId, Order = 1, TopicId = Guid.NewGuid()},
                new Topic() {TopicName = "Nuget", TopicContext = NugetContext, DateAdded = DateTime.Now, CategoryId = UtilCategoryId, Order = 2, TopicId = Guid.NewGuid()}
            };
